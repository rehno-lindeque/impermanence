...
****** ~files~
       ...
       - ~method~, the method used to link the file to persistent
         ...
         exists in persistent storage and a symlink otherwise.
         ~"reconcile"~ behaves like ~"auto"~, but safely adopts an
         already-existing file at the mount point by moving it into
         persistent storage when the target is missing, or replacing it
         when both files already match. ~"symlink"~ uses a symlink
         regardless. Only change this from ~"auto"~ if it is giving you
         issues.
